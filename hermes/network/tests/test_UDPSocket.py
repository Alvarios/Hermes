...
def test_socket_can_received_multiple_message_while_running():
    ...
    # When
    udp_socket = UDPSocket(socket_ip=socket_ip, socket_port=socket_port, max_queue_size=n_msg).start()
...
def test_change_key_correctly_change_the_fernet_encoder():
    ...
    msg_crypt = udp_socket.fernet_encoder._encrypt(msg)
    msg_decrypt_old = udp_socket.fernet_encoder._decrypt(msg_crypt)
    udp_socket.change_key(key_new)

    # Then
    assert msg_decrypt_old == msg
    with pytest.raises(InvalidToken):
        udp_socket.fernet_encoder._decrypt(msg_crypt)
    udp_socket.stop()
    time.sleep(.5)
